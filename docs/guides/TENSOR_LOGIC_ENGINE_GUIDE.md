...
**特徴:**
- ✅ データ駆動: 既知の事実からすべての結論を導出
- ✅ 網羅的: すべての導出可能な事実を生成
- ❌ 効率: 不要な推論も実行される可能性

#### 後向き推論アルゴリズム（Backward Chaining）

```
入力: 目標 G、事実集合 F、ルール集合 R
出力: 目標達成の可否と推論パス
...
2. G を生成できるルール r を探す:
   a. r.output == G となる r を見つける
   b. r の各入力 i に対して:
      - 再帰的に backwardChain(i) を実行
   c. すべての入力が解決できれば:
      - r を適用して G を計算
      → 成功を返す
...
詳細は [BACKWARD_CHAINING_GUIDE.md](BACKWARD_CHAINING_GUIDE.md) を参照してください。
